...
$post = $pdo->prepare("SELECT * FROM posts where published = 0 order by id desc");
$post->execute();
...
<div class="card-container">
    <?php foreach ($posts as $serv): ?>
        <div class="card">
            <img src="images/<?php echo $serv->image ?>" alt="<?php echo $serv->title ?>" class="card-img">
            <div class="card-info">
                <h3 class="card-title"><?php echo $serv->title ?></h3>
            </div>
            <?php if($_SESSION['login'] == "admin"): ?>
                <div class="admin-buttons">
                    <form style="text-align: center;" action="realising/PostActions.php" method="post">
                        <input type="hidden" name="id_to_publish" value="<?php echo $serv->id; ?>">
